...
class Header(object):
    ...
    def parse (self):
        ...
        # document ID and unique ID
        if globals.params.oleConsistency:
            if globals.indexbytes(self.bytes, 0) != 0xd0 or \
                   globals.indexbytes(self.bytes, 1) != 0xcf or \
                   globals.indexbytes(self.bytes, 2) != 0x11 or \
                   globals.indexbytes(self.bytes, 3) != 0xe0 or \
                   globals.indexbytes(self.bytes, 4) != 0xa1 or \
                   globals.indexbytes(self.bytes, 5) != 0xb1 or \
                   globals.indexbytes(self.bytes, 6) != 0x1a or \
                   globals.indexbytes(self.bytes, 7) != 0xe1:
                raise Exception("OLE: bad signature")
        ...
        if globals.params.oleConsistency and self.secSize > 18:
            raise Exception("Ole: bad sector size exp. %d" % self.secSize)
        # short sector size (usually 64 bytes)
        self.secSizeShort = getSignedInt(self.bytes[32:34])
        if globals.params.oleConsistency and self.secSizeShort > 18:
            raise Exception("Ole: bad short sector size exp. %d" %
                            self.secSizeShort)
...
"""
    ...
    def outputRawBytes (self):
        ...
        for secID in self.sectorIDs:
            pos = 512 + secID*self.sectorSize
            bytes.append(self.bytes[pos:pos+self.sectorSize])
        globals.dumpBytes(b''.join(bytes), 512)
...
"""
    ...
    def __getRawStream (self, entry):
        ...
        if entry.StreamLocation == StreamLocation.SSAT:
            ...
            for id in chain:
                pos = id*size
                bytes.append(self.RootStorageBytes[pos:pos+size])
            return b''.join(bytes)
        ...
        for id in chain:
            pos = offset + id*size
            bytes.append(self.header.bytes[pos:pos+size])
