...
def parse_opf(xml_string):
    ...
    data = {
        k: package.getElementsByTagNameNS(XMLNS_OPF, k).item(0)
        for k in ('metadata', 'manifest', 'spine', 'guide')
    }
...
def _parse_xml_metadata(element):
    ...
    for node in element.getElementsByTagNameNS(XMLNS_DC, 'title'):
        metadata.add_title(get_node_text(node),
                           node.getAttribute('xml:lang'))

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'creator'):
        metadata.add_creator(get_node_text(node),
                             node.getAttributeNS(XMLNS_OPF, 'role'),
                             node.getAttributeNS(XMLNS_OPF, 'file-as'))

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'subject'):
        metadata.add_subject(get_node_text(node))

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'description'):
        metadata.description = get_node_text(node)

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'publisher'):
        metadata.publisher = get_node_text(node)

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'contributor'):
        metadata.add_contributor(get_node_text(node),
                                 node.getAttributeNS(XMLNS_OPF, 'role'),
                                 node.getAttributeNS(XMLNS_OPF, 'file-as'))

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'date'):
        metadata.add_date(get_node_text(node),
                          node.getAttributeNS(XMLNS_OPF, 'event'))

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'type'):
        metadata.dc_type = get_node_text(node)

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'format'):
        metadata.format = get_node_text(node)

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'identifier'):
        metadata.add_identifier(get_node_text(node),
                            node.getAttribute('id'),
                            node.getAttributeNS(XMLNS_OPF, 'scheme'))

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'source'):
        metadata.source = get_node_text(node)

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'language'):
        metadata.add_language(get_node_text(node))

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'relation'):
        metadata.relation = get_node_text(node)

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'coverage'):
        metadata.coverage = get_node_text(node)

    for node in element.getElementsByTagNameNS(XMLNS_DC, 'rights'):
        metadata.right = get_node_text(node)

    for node in element.getElementsByTagNameNS(XMLNS_OPF, 'meta'):
...
def _parse_xml_manifest(element):
    ...
    manifest = Manifest()
    for e in element.getElementsByTagNameNS(XMLNS_OPF, 'item'):
...
def _parse_xml_spine(element):
    ...
    for e in element.getElementsByTagNameNS(XMLNS_OPF, 'itemref'):
...
def _parse_xml_guide(element):
    ...
    guide = Guide()
    for e in element.getElementsByTagNameNS(XMLNS_OPF, 'reference'):
