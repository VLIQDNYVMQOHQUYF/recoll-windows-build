...
XMLNS_CONTAINER = 'urn:oasis:names:tc:opendocument:xmlns:container'
...
class EpubFile(zipfile.ZipFile):
    ...
    def _init_read(self):
        ...
        for element in container_xml.getElementsByTagNameNS(XMLNS_CONTAINER, 'rootfile'):
    ...
    def _write_close(self):
        ...
        if item_toc:
            to_remove.append(
                posixpath.join(self.content_path, item_toc.href)
            )
        ...
        if item_toc:
            toc_path = posixpath.join(self.content_path, item_toc.href)
    ...
    def add_item(self, filename, manifest_item,
        ...
        write_path = posixpath.join(self.content_path, manifest_item.href)
    ...
    def extract_item(self, item, to_path=None):
        ...
        member_path = posixpath.join(self.content_path, path)
    ...
    def read_item(self, item):
        ...
        path = posixpath.join(self.content_path, path)
        path = posixpath.normpath(path)
        # Replace \ by /, as ZipFile always uses / as path separator.
        path = urllib_unquote(path.replace('\\', '/'))

        try:
            return self.read(path)
        except KeyError:
            path = path.encode().decode('cp437')
            try:
                return self.read(path)
            except KeyError:
                pass
