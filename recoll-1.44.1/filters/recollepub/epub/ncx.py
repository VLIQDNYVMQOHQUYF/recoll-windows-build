...
XMLNS_NCX = 'http://www.daisy.org/z3986/2005/ncx/'
...
def parse_toc(xmlstring):
    ...
    version = toc_xml.getAttributeNS(XMLNS_NCX, 'version')
    if version:
        toc.version = version

    lang = toc_xml.getAttributeNS(XML_NAMESPACE, 'lang')
    if lang:
        toc.lang = lang
    ...
    metas = {'dtb:uid': '',
             ...
             'dtb:maxPageNumber': '',
             'dtb:generator': ''}
    head = toc_xml.getElementsByTagNameNS(XMLNS_NCX, 'head')
    if len(head) > 0:
        for meta in head[0].getElementsByTagNameNS(XMLNS_NCX, 'meta'):
            metas[meta.getAttributeNS(XMLNS_NCX, 'name')] = meta.getAttributeNS(XMLNS_NCX, 'content')
    ...
    doc_title_nodes = toc_xml.getElementsByTagNameNS(XMLNS_NCX, 'docTitle')
    if len(doc_title_nodes) == 1:
        toc.title = _parse_for_text_tag(doc_title_nodes[0])
    else:
        msg = 'The NCX should define exactly one docTitle ' \
              '(the ePub defines {})'.format(len(doc_title_nodes))
        warnings.warn(msg, SyntaxWarning)
        toc.title = ''

    # Get authors (<docAuthor> tags are optionnal)
    for author in toc_xml.getElementsByTagNameNS(XMLNS_NCX, 'docAuthor'):
        toc.authors.append(_parse_for_text_tag(author))

    # Inspect <navMap> (one is required)
    nav_map_nodes = toc_xml.getElementsByTagNameNS(XMLNS_NCX, 'navMap')
    if len(nav_map_nodes) == 1:
        toc.nav_map = _parse_xml_nav_map(nav_map_nodes[0])
    else:
        msg = 'The NCX should define exactly one navMap ' \
              '(the ePub defines {})'.format(len(nav_map_nodes))
        warnings.warn(msg, SyntaxWarning)
        toc.nav_map = NavMap()

    # Inspect <pageList> (optionnal, only one)
    page_lists = toc_xml.getElementsByTagNameNS(XMLNS_NCX, 'pageList')
    if len(page_lists) > 0:
        toc.page_list = _parse_xml_page_list(page_lists[0])

    # Inspect <navList> (optionnal, many are possible)
    for nav_list in toc_xml.getElementsByTagNameNS(XMLNS_NCX, 'navList'):
...
def _parse_xml_nav_map(element):
    ...
    nav_map = NavMap()
    nav_map.identifier = element.getAttributeNS(XMLNS_NCX, 'id')
    ...
    for node in children:
        if node.tagName == 'navLabel':
            nav_map.add_label(_parse_for_text_tag(node),
                              node.getAttributeNS(XML_NAMESPACE, 'lang'),
                              node.getAttributeNS(XMLNS_NCX, 'dir'))
...
def _parse_xml_nav_point(element):
    ...
    """
    nav_point = NavPoint()
    nav_point.identifier = element.getAttributeNS(XMLNS_NCX, 'id')
    nav_point.class_name = element.getAttributeNS(XMLNS_NCX, 'class')
    nav_point.play_order = element.getAttributeNS(XMLNS_NCX, 'playOrder')
...
def _parse_xml_page_list(element):
    ...
    """
    page_list = PageList()
    page_list.identifier = element.getAttributeNS(XMLNS_NCX, 'id')
    page_list.class_name = element.getAttributeNS(XMLNS_NCX, 'class')
...
def _parse_xml_page_target(element):
    ...
    page_target.identifier = element.getAttributeNS(XMLNS_NCX, 'id')
    page_target.value = element.getAttributeNS(XMLNS_NCX, 'value')
    page_target.target_type = element.getAttributeNS(XMLNS_NCX, 'type')
    page_target.class_name = element.getAttributeNS(XMLNS_NCX, 'class')
    page_target.play_order = element.getAttributeNS(XMLNS_NCX, 'playOrder')
...
def _parse_xml_nav_list(element):
    ...
    nav_list = NavList()
    nav_list.identifier = element.getAttributeNS(XMLNS_NCX, 'id')
    nav_list.class_name = element.getAttributeNS(XMLNS_NCX, 'class')
...
def _parse_xml_nav_target(element):
    ...
    nav_target.identifier = element.getAttributeNS(XMLNS_NCX, 'id')
    nav_target.value = element.getAttributeNS(XMLNS_NCX, 'value')
    nav_target.class_name = element.getAttributeNS(XMLNS_NCX, 'class')
    nav_target.play_order = element.getAttributeNS(XMLNS_NCX, 'playOrder')
...
class Ncx(object):
    ...
    def as_xml_document(self):
        ...
        ncx.setAttribute('xmlns', XMLNS_NCX)
